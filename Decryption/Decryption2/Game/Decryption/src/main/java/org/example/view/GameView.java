...
import javafx.scene.text.FontWeight;đ
import javafx.scene.text.Text;
import javafx.scene.text.TextAlignment;
import javafx.util.Duration;
import org.example.model.entity.GameState;
...
public class GameView {
    ...
    private void initializeUI() {
        ...
        wordListContainer.setMinWidth(200);
        wordListContainer.setPrefWidth(250);
        wordListContainer.getStyleClass().add("word-list-container");

        // Add a placeholder label to the word list container
        Label selectWordLabel = new Label("SELECT A WORD:");
        selectWordLabel.setFont(Font.font("Arial", FontWeight.BOLD, 14));
        wordListContainer.getChildren().add(selectWordLabel);

        // Attempts container
        attemptsContainer = new VBox(5);
        attemptsContainer.setAlignment(Pos.TOP_LEFT);
        attemptsContainer.setPadding(new Insets(10));
        attemptsContainer.setMinWidth(200);
        attemptsContainer.setPrefWidth(250);
        attemptsContainer.getStyleClass().add("attempts-container");

        // Add a placeholder label to the attempts container
        Label attemptsLabel = new Label("YOUR ATTEMPTS:");
        attemptsLabel.setFont(Font.font("Arial", FontWeight.BOLD, 14));
        attemptsContainer.getChildren().add(attemptsLabel);
        ...
        // Style the containers with borders
        wordListContainer.setStyle("-fx-border-color: #cccccc; -fx-border-width: 1; -fx-border-radius: 5; -fx-background-color: #f8f8f8; -fx-background-radius: 5;");
        attemptsContainer.setStyle("-fx-border-color: #cccccc; -fx-border-width: 1; -fx-border-radius: 5; -fx-background-color: #f8f8f8; -fx-background-radius: 5;");

        // Add containers to grid with constraints to fill the columns
        centerGrid.add(wordListContainer, 0, 0);
        centerGrid.add(attemptsContainer, 1, 0);

        // Set these containers to grow vertically
        GridPane.setVgrow(wordListContainer, Priority.ALWAYS);
        GridPane.setVgrow(attemptsContainer, Priority.ALWAYS);
        GridPane.setFillHeight(wordListContainer, true);
        GridPane.setFillHeight(attemptsContainer, true);
    ...
    private VBox createInfoSection() {
        ...
        infoBox.setStyle("-fx-border-color: #cccccc; -fx-border-width: 1; -fx-border-radius: 5; -fx-background-color: #f8f8f8; -fx-background-radius: 5;");
    ...
    public void updateView(GameState state) {
        ...
        // Add word buttons
        if (state.getWordOptions() != null && !state.getWordOptions().isEmpty()) {
            for (String word : state.getWordOptions()) {
                Button wordButton = new Button(word);
                wordButton.setMaxWidth(Double.MAX_VALUE);
                wordButton.setPrefHeight(35);
                wordButton.setStyle("-fx-font-size: 14px; -fx-font-family: 'Monospace';");
                ...
                if (state.getAttemptedWords().contains(word) || state.isGameOver()) {
                    ...
                    wordButton.setStyle(wordButton.getStyle() + "-fx-opacity: 0.7;");
                }
            ...
            // Add a placeholder message if no words are available
            Label noWordsLabel = new Label("No words available.");
            noWordsLabel.setFont(Font.font("Arial", FontStyle.ITALIC, 14));
            wordListContainer.getChildren().add(noWordsLabel);
        }
    ...
    private void updateAttemptsDisplay(GameState state) {
        ...
        if (attemptedWords.isEmpty()) {
            // Add a placeholder message if no attempts have been made
            Label noAttemptsLabel = new Label("No attempts yet.");
            noAttemptsLabel.setFont(Font.font("Arial", FontStyle.ITALIC, 14));
            attemptsContainer.getChildren().add(noAttemptsLabel);

            // Add remaining attempts info
            Label remainingLabel = new Label("Remaining attempts: " + state.getMaxAttempts());
            remainingLabel.setFont(Font.font("Arial", 14));
            remainingLabel.setPadding(new Insets(10, 0, 0, 0));
